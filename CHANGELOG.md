...
### 改进：邮件和 Telegram 通知使用完整报告格式

邮件和 Telegram 通知现在使用与 Markdown 报告文件相同的格式，提供更丰富和结构化的内容展示。

### 主要变更

**Telegram 通知**
- ✅ 使用完整的分析内容（不再截断）
- ✅ 移除复杂的 Markdown V2 转义（之前经常导致解析错误）
- ✅ 使用纯文本格式，更可靠
- ✅ 智能分段：超过 4096 字符时按行分割，保持内容完整性
- ✅ 添加生成时间和分隔线，与 Markdown 报告格式一致

**邮件通知**
- ✅ 全新的 HTML 样式设计，更现代美观
- ✅ 完整的分析内容展示（不再截断）
- ✅ 改进的元数据展示（日期、账号数、推文数、生成时间）
- ✅ 纯文本版本也使用完整报告格式
- ✅ 响应式设计，适配各种邮件客户端

### 格式对比

**之前：** 简单的标题 + 关键洞察列表 + 截断的分析（3000字符）  
**现在：** 完整的结构化报告，包含所有分析维度和详细内容

### 修改的文件

1. **src/notifiers/telegram_notifier.py**
   - 简化消息格式，移除 Markdown V2 转义
   - 使用与 output 报告相同的格式模板
   - 改进长消息分割逻辑

2. **src/notifiers/email_notifier.py**
   - 重新设计 HTML 模板（更现代的样式）
   - 更新纯文本版本格式
   - 完整展示所有分析内容
...
- ✅ **内容完整** - 不再因字符限制截断重要信息
- ✅ **格式统一** - 邮件、Telegram、Markdown 文件三者格式一致
- ✅ **更稳定** - 移除了 Telegram Markdown 解析错误
- ✅ **更美观** - 邮件 HTML 样式全面升级
